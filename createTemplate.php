        ...
        foreach ($this->fields as $key => $val) {
            ...
            if (strpos($val, '1')) {
                ...
                foreach ($temp as $item) {
                    $assign = explode(":", $item);
                    $assignList .= "$spilt{$assign[0]}=>'" . trim(end($assign)) . "'";
                    $spilt=',';
                }
                $assignList .= "][";
            }
            ...
            if (empty($assignList)) {
                ...
                $fieldList[$key] = '                        <td>' . $assignList.'$item["' . $key . '"]];?>' . '</td>';
            }
        }
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="' . $key . '">
                                <?php
                                    ' . $assignList . '
                                    foreach(' . "$" . $key . "List" . ' as $key=>$val){ ?>
                                        <option <?php $info[\''.$key.'\'] == $key &&  print(\'selected\');?>  value="<?php echo $key?>"><?php echo $val?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
